...
Safety:
    ...
    - Treats time1.outTime as Maya's known default image-sequence connection.
    - Reuses an existing expression only when it clearly drives frameExtension.
    - Reset removes only the assignment managed by this tool and restores 1-frame playback.
"""
...
def _is_default_time_source(source_plug: str) -> bool:
    """Return True only for Maya's standard image-sequence time driver."""
    return source_plug == "time1.outTime"


def _disconnect_default_time_source(target_attr: str) -> List[str]:
    """Disconnect Maya's known time1.outTime driver and return removed plugs."""
    removed = []

    for source in _incoming_sources(target_attr):
        if _is_default_time_source(source):
            try:
                cmds.disconnectAttr(source, target_attr)
                removed.append(source)
            except Exception as exc:
                raise RuntimeError(
                    f"既定接続を解除できません: {source} -> {target_attr}\n{exc}"
                )

    return removed


def _restore_default_time_source(target_attr: str) -> bool:
    """Restore Maya's normal one-image-per-frame connection when safe."""
    incoming = _incoming_sources(target_attr)

    if "time1.outTime" in incoming:
        return False

    if incoming:
        raise RuntimeError(
            "通常1コマ打ちへ戻せません。未知の接続があります。\n"
            + "\n".join(incoming)
        )
...
def apply_hold(*_args) -> None:
    ...
    # Enabling Use Image Sequence in Maya 2026 normally connects
    # time1.outTime -> frameExtension. This is safe to replace.
    unknown_incoming = [
        source
        for source in incoming
        if not _is_default_time_source(source)
    ]

    if unknown_incoming and not assignments:
        message = (
            "適用を中止しました。\n"
            "frameExtensionに未知の接続があります。\n"
            "自動削除は行いません。\n"
            "接続元:\n  - "
            + "\n  - ".join(unknown_incoming)
        )
    ...
    try:
        removed_default_sources = _disconnect_default_time_source(target_attr)
    except Exception as exc:
        cmds.warning(str(exc))
        _set_log(f"適用失敗: 既定接続を解除できません。\n{exc}")
        return
    ...
    removed_text = (
        ", ".join(removed_default_sources)
        if removed_default_sources
        else "なし"
    )
...
def reset_hold(*_args) -> None:
    ...
    if not assignments:
        try:
            cmds.setAttr(f"{shape}.useFrameExtension", True)
            restored = _restore_default_time_source(target_attr)
        except Exception as exc:
            cmds.warning(str(exc))
            _set_log(f"通常1コマ打ちへ戻せません。\n{exc}")
            return

        restore_text = "再接続しました" if restored else "すでに接続済みです"
        _set_log(
            "通常の1コマ打ちを確認しました。\n"
            f"対象: {shape}\n"
            f"time1.outTime: {restore_text}\n"
    ...
    try:
        ...
        # Reset means restore Maya's normal one-image-per-frame sequence.
        cmds.setAttr(f"{shape}.useFrameExtension", True)
        restored = _restore_default_time_source(target_attr)
...
def show() -> None:
    ...
    cmds.button(label="通常1コマへ戻す", command=reset_hold)
